...
1. atomic vector, matrix, array 都要求单一数据类型 (homogeneous data type)，均由 **atomic vector** 派生
   1. atomic vector 添加维度属性可生成 matrix, array
   2. atomic vector 添加 class 属性可生成 factor, date 和 date-time
2. list, data.frame 可以装载不同数据类型 (heterogeneous data types)，均由 **list** 派生
...
`c()`串联合并向量（将多个单元素向量和多元素向量连接起来构成一个向量），其他语言一般用`[]`

`c()`中即使嵌套使用`c()`，也会被自动展开为 atomic vector
...
`m:n`创建连续整数向量，类型为 integer vector
...
R 对向量的大部分操作都在原向量的拷贝上进行，不改变原向量（只有直接赋值能改变原向量）。好处是安全，坏处是慢。


#### 增删改

若要直接改变原向量，只需对原向量或其子集直接赋值；对不存在的位置赋值时，该位置前面未被定义的元素默认用 NA 补齐。
...
- `sort(x, decreasing = FALSE, na.last = FALSE, ...)`返回排序后的向量，对于数字默认升序，对于字符串默认字典顺序
- `rev(x)`逆序排列
- `rank(x)`返回x中各元素相对大小的index（默认升序）
- `order(x)`返回按照递增/递减的次序重排列x后，新序列各元素在原序列x中的位置。`x[order(x)]`等同于`sort(x)`
...
`matrix()` 的 dimnames 参数，格式为 list(row_name_vector, col_name_vector)
